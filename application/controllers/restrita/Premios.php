...
class Premios extends CI_Controller {
	...
	public function index(){
		$data = array(
				'titulo' => 'Prêmios',
				'subtitulo' => 'Lista de Prêmios',
				'scripts' => array(
					'js/data-table.js',
					'js/alerts.js'
				),
				'premios' => $this->core_model->getall('premios')
			);
		$this->load->view('restrita/template/header', $data);
		$this->load->view('restrita/premios/index');
	...
	public function cadastrar(){
		...
		if($this->form_validation->run()){
			...
			if($this->core_model->cadastrar('premios', $data)){
				$this->session->set_flashdata('sucesso', 'Dados salvos com sucesso!');
			}else{
				$this->session->set_flashdata('erro', 'Não foi possível cadastrar o prêmio!');
			}
				
			redirect('restrita/premios');
		}else{
			$data = array(
				'titulo' => 'Prêmios',
				'subtitulo' => 'Cadastrar prêmio',
				'scripts' => array(
					'js/file-upload.js',
					'jquery-upload-file/js/jquery.uploadfile.min.js',
					'jquery-upload-file/js/premios.js',
			...
			$this->load->view('restrita/template/header', $data);
			$this->load->view('restrita/premios/cadastrar');
			$this->load->view('restrita/template/footer');
		}
	}
	...
	public function editar($chave_id){
		...
		if($this->form_validation->run()){
			...
			if($this->core_model->editar('premios', array('id' => $chave_id),$data)){
				$this->session->set_flashdata('sucesso', 'Dados salvos com sucesso!');
			}else{
				$this->session->set_flashdata('erro', 'Não foi possível atualizar o prêmio!');
			}
			...
			$data = array(
				...
				'premio' => $this->core_model->getby('premios', array('id' => $chave_id)),
			);

			$this->load->view('restrita/template/header', $data);
			$this->load->view('restrita/premios/editar');
			$this->load->view('restrita/template/footer');
		}
	}

	public function delete($chave_id){
		if($this->core_model->deletar('premios', array('id' => $chave_id))){
	...
	public function upload($imagem){
		...
		$config['upload_path']          = './uploads/sobre/premios/';
